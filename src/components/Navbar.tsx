    ...
    <header className="sticky top-0 z-50 w-full border-b border-border dark:border-dark-border bg-surface/80 dark:bg-dark-base/80 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <span
              className={`block w-5 h-0.5 bg-text-primary dark:bg-dark-text-primary transition-all duration-300 origin-center ${menuOpen ? "rotate-45 translate-y-2" : ""}`}
            />
            <span
              className={`block w-5 h-0.5 bg-text-primary dark:bg-dark-text-primary transition-all duration-300 ${menuOpen ? "opacity-0 scale-x-0" : ""}`}
            />
            <span
              className={`block w-5 h-0.5 bg-text-primary dark:bg-dark-text-primary transition-all duration-300 origin-center ${menuOpen ? "-rotate-45 -translate-y-2" : ""}`}
            />
      ...
      {/* Mobile Menu */}
      <div
        className={`lg:hidden overflow-hidden transition-all duration-300 ease-in-out ${menuOpen ? "max-h-128 opacity-100" : "max-h-0 opacity-0"}`}
      >
        <div className="max-w-7xl mx-auto px-6 pb-6 pt-2 flex flex-col gap-1 border-t border-border dark:border-dark-border bg-surface dark:bg-dark-base">
          {NAV_LINKS.map(({ label, to }) => (
            <NavLink
              ...
              onClick={() => setMenuOpen(false)}
              className={({ isActive }) =>
                `py-3.5 text-sm font-medium border-b border-border dark:border-dark-border transition-colors duration-200 ${
                  isActive
                    ? "text-text-primary dark:text-dark-text-primary"
